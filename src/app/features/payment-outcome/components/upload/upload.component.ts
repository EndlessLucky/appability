...
@Component({
    selector: 'app-payment-outcome-upload',
    templateUrl: './upload.component.html',
    styleUrls: ['./../../../../shared/grid/components/core-form/core-form.component.scss']
})
export class UploadComponent extends BaseFormComponent {
    ...
    //     {value: 'module', label: 'Module', route: 'module'},
    // ];
    //
    ...
    private upload() {
        ...
        this.files.forEach(file => {
            // console.warn(file);
            this.callUploadService(file, 'payment_outcome');
        });
                ...
                if (event.body.success) {
                    if (event.body.isImage) {
                        ...
                        if (event.body.rows) {
                          ...
                          msg = 'File uploaded: ' + event.body.file.name + ' \n' + event.body.totalRows + ' row(s) processed.';
                          if (event.body.unpaidList && event.body.unpaidList.length) {
                            this.unpaidList = event.body.unpaidList;
                          }
                          if (event.body.paidList && event.body.paidList) {
                            this.paidList = event.body.paidList;
                          }
                        }
                    }
                    ...
                    if (event.body.isImage) {
                        msg = 'Image upload error: ' + event.body.errorMessage;
                        console.log(event.body);
                    } else {
                      msg = 'Import error: ' + event.body.errorMessage;
                    }
